...
namespace EasySurvivalScripts
{
    ...
    public class PlayerCamera : MonoBehaviour
    {
        ...
        void RotateCamera()
        {
            float mouseX = bl_TouchPad.Horizontal * (mouseSensitivity * Time.deltaTime);
            float mouseY = bl_TouchPad.Vertical * (mouseSensitivity * Time.deltaTime);
            Vector3 eulerRotation = transform.eulerAngles;

            xClamp += mouseY;

            if(cameraPerspective == CameraPerspective.FirstPerson)
                xClamp = Mathf.Clamp(xClamp, FPS_MinMaxAngles.x, 40.0f);
            else
